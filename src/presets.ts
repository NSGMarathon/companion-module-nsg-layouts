...
export function getPresetDefinitions(
	...
	const interstitialVideoPresets = (socket.replicants[LAYOUT_BUNDLE_NAME].videoFiles?.interstitials ?? []).reduce(
		(result, video) => {
			result[`interstitial_play_${video.name.replaceAll(' ', '_')}`] = {
				...
				name: video.name,
				style: {
					text: `${video.name}\\nPlayed $(${instance.label}:interstitial_last_played_${video.name.replaceAll(
						' ',
						'_'
					)})`,
					size: 'auto',
